...
def custom_calculate_taxes(doc, method):
	...
	tax_calculator = calculate_taxes_and_totals(doc)
	tax_calculator.calculate_totals()

	# Add your custom tax logic here
	if doc.taxes:
		for row in doc.taxes:
			if "TDS" in row.account_head:
...
class calculate_taxes_and_totals:
	...
	# 			print(row.account_head,'account_headaccount_head')
	...
	# 		"POS Invoice",
	# 	]:
	# 		print('hasjfhafjkafnhjdad,m',self.doc.grand_total * self.doc.conversion_rate, self.doc.precision("base_grand_total"))
	...
	def calculate_totals(self):
		if self.doc.get("taxes"):
		    # Exclude TDS rows while calculating total taxes
		    non_tds_taxes = [row for row in self.doc.get("taxes") if "TDS" not in row.account_head]


		    self.doc.grand_total = flt(non_tds_taxes[-1].total if non_tds_taxes else 0) + flt(
		        self.doc.get("grand_total_diff")
		    )
		else:
		    self.doc.grand_total = flt(self.doc.net_total)
		...
		if self.doc.doctype in [
		    ...
		    self.doc.taxes_and_charges_added = self.doc.taxes_and_charges_deducted = 0.0
		    for tax in non_tds_taxes:  # Only consider non-TDS taxes
		        if tax.category in ["Valuation and Total", "Total"]:
		            if tax.add_deduct_tax == "Add":
		                self.doc.taxes_and_charges_added += flt(tax.tax_amount_after_discount_amount)
		            else:
