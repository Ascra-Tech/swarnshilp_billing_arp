...
frappe.ui.form.on('Sales Receipt Voucher', {
	refresh(frm) {
        setStatusPill(frm)
	    setQueryFilter(frm);
...
function setQueryFilter(frm){
    frappe.call({
        method: "ascra_billing.ascra_billing.doctype.sales_receipt_voucher.sales_receipt_voucher.get_address_by_account_code",
         args: {
...
function addActionButton(frm){
    ...
    frm.page.add_action_item(__("Reject"), function() {
        ...
        frappe.call({
            method: "ascra_billing.ascra_billing.doctype.sales_receipt_voucher.sales_receipt_voucher.set_status",
             args: {
